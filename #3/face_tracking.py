    ...
    try:
        ...
        if not ret_val:
            ...
            if len(p0) <= 10:
                ...
                foundP = np.zeros_like(p1)
                counter = 0
                for k in range(len(isFound)):
                    if isFound[k] == 1:
                        foundP[counter] = p1[k]
                        counter = counter + 1;
                temp = 0
                for element in foundP:
                    if element[0] == 0 and element[1] == 0:
                        break;
                    temp = temp + 1
                foundP = foundP[0:temp]
                print("foundP", foundP)
                ##-your-code-end-here-##

                # Draw points using e.g. cv2.drawMarker
                ##-your-code-starts-here-##
                for i in range(len(foundP)):
                    img = cv2.drawMarker(img, (int(foundP[i, 0]), int(foundP[i, 1])), color=(0, 255, 0))
                ##-your-code-ends-here-##
