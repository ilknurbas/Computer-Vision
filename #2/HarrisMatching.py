...
def harris(im, sigma=1.0, rel_thresh=0.01, k=0.04):
    im = im.astype(np.float32)  # Make sure im is float
...
for i in range(npts1):
    ...
    patches1[:, :, i] = patch
    m_1[i] = sum(sum(patches1[:, :, i])) / (np.square(patch_size))

for i in range(npts2):
    patch = map_coordinates(I2, (yv + y2[i], xv + x2[i]))
    patches2[:, :, i] = patch
    m_2[i] = sum(sum(patches2[:, :, i])) / (np.square(patch_size))
...
for i1 in range(npts1):
    ...
    for i2 in range(npts2):
        mean_2 = m_2[i2]
        up = sum(sum((patches1[:, :, i1] - mean_1) * (patches2[:, :, i2] - mean_2)))
        down = np.sqrt(
            sum(sum(np.square(patches1[:, :, i1] - mean_1))) * sum(sum(np.square(patches2[:, :, i2] - mean_2))))
        ncc[i1, i2] = up / down

# Next, compute pairs of patches that are mutually nearest neighbors
# according to the SSD measure
ss1_ncc = np.amax(ncc, axis=1)
ids1_ncc = np.argmax(ncc, axis=1)
ss2_ncc = np.amax(ncc, axis=0)
ids2_ncc = np.argmax(ncc, axis=0)

pairs_ncc = []
for k in range(npts1):
    if k == ids2_ncc[ids1_ncc[k]]:
        pairs_ncc.append(np.array([k, ids1_ncc[k], ss1_ncc[k]]))
pairs_ncc = np.array(pairs_ncc)

# We sort the mutually nearest neighbors based on the SSD
sorted_ncc = np.sort(pairs_ncc[:, 2], axis=0)
id_ncc = np.argsort(pairs_ncc[:, 2], axis=0)
sorted_ncc = sorted_ncc[::-1]
id_ncc = id_ncc[::-1]
pairs = pairs_ncc
##-your-code-ends-here-##
