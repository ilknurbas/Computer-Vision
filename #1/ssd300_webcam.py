...
while True:
    ...
    fram = img
    # 2.
    # uncomment if you are using webcam
    img = cv2.flip(img, 1)
    # 3.
    pred_img = imresize(img, [img_height, img_width], preserve_range=True)
    # 4.
    pred_img = np.array([pred_img])
    # 5.
    y = model.predict(pred_img)
    # print("y", y[0])
    # y = y[y[:, :, 1] > 0.0, :]
    y = y[y[:, :, 1] > 0.0, :]
    # 6.
    for k in y:
        print(k)
        x1 = int(k[2] * img.shape[1] / img_width)
        y1 = int(k[3] * img.shape[0] / img_height)
        x2 = int(k[4] * img.shape[1] / img_width)
        y2 = int(k[5] * img.shape[0] / img_height)
        ...
        #      Class labels are defined above, and can be indexed with the class_id
        #      in the predictions.
        img = cv2.putText(img, classes[int(k[0])], (x1, y1), cv2.FONT_HERSHEY_SIMPLEX,
